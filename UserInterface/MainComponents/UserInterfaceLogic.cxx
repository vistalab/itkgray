...
// We need this to compute topology stats updates
...
::InitializeActivationFlags()
{
  ...
  m_Activation->AddMenuItem(m_MenuUndo, UIF_UNDO_POSSIBLE);
  m_Activation->AddMenuItem(m_MenuRedo, UIF_REDO_POSSIBLE);
...
::InitializeUI()
{
  ...
  m_InPaintbrushTolerance->value(pbs.tolerance);
  m_InPaintbrushSmoothing->value(pbs.smoothing);

  if(pbs.shape == PAINTBRUSH_FLOODFILL)
    {
    m_InPaintbrushShape->value(m_ChoicePaintbrushFloodfill);
    m_InPaintbrushTolerance->activate();
    m_InPaintbrushSmoothing->activate();
    }
  else
    {
    m_InPaintbrushTolerance->deactivate();
    m_InPaintbrushSmoothing->deactivate();
...
::UpdateImageProbe() 
{
  ...
  sGrey << m_Driver->GetCurrentImageData()->GetGrey()->GetRealVoxelIntensity(crosshairs);
...
::OnPaintbrushAttributesUpdate()
{
  ...
  pbs.tolerance = m_InPaintbrushTolerance->value();
  pbs.smoothing = m_InPaintbrushSmoothing->value();

  if(m_InPaintbrushShape->value() == 2)
    {
    pbs.shape = PAINTBRUSH_FLOODFILL;
    m_InPaintbrushTolerance->activate();
    m_InPaintbrushSmoothing->activate();
    }
  else
...
::OnPaintbrushPaint()
{
  // The user painted something. We should make it possible to modify the mesh
  m_Activation->UpdateFlag(UIF_IRIS_MESH_DIRTY, true);
}

/**
 * Handle Topology Stats callbacks
 */
void 
UserInterfaceLogic
::OnGatherStatistics()
{
   int nVol, nCav, nHan;
   double euler;
   m_Driver->GetCurrentImageData()->GetSegmentation()->EulerStat(m_GlobalState->GetDrawingColorLabel(), &euler, &nCav, &nVol, &nHan);
   char *str = new(char [20]);
   sprintf(str,"%d",nVol);
   m_OutStatsNumVols->value(str);
   sprintf(str,"%d",nCav);
   m_OutStatsNumCavities->value(str);
   sprintf(str,"%d",nHan);
   m_OutStatsNumHandles->value(str);
   sprintf(str,"%0.2f",euler);
   m_OutStatsEuler->value(str);
   //m_OutStatsHandles->clear();
   // Activatge/dectivate logic for buttons
   if(nVol>1) m_DelSatelliteVol->activate();
   else m_DelSatelliteVol->deactivate();
   if(nCav>0) m_FillCavities->activate();
   else m_FillCavities->deactivate();
   if(nHan>0 && nCav==0 && nVol==1) m_FindHandles->activate();
   else{ m_FindHandles->deactivate(); m_OutStatsHandles->clear(); }
}
...
::OnFindHandles()
{
   Loc *han_list = m_Driver->GetCurrentImageData()->GetSegmentation()->FindHandles(m_GlobalState->GetDrawingColorLabel());
   m_OutStatsHandles->clear();
   if(han_list == NULL)
   {
     m_OutStatsHandles->value(m_OutStatsHandles->add("none",0,0,0,0));
     m_MenuPrevHandle->deactivate();
     m_MenuNextHandle->deactivate();
   }
   else
   {
     int i = 0;
     ...
     do
     {
       /* ending condition to exit the loop*/
       if(han_list[i].x == 1023 && han_list[i].y == 1023 && han_list[i].z == 1023) break;
       snprintf(s, 64, "%d, %d, %d", han_list[i].x, han_list[i].y, han_list[i].z);
       m_OutStatsHandles->add(s,0,0,0,0);
       i++;
     }while(1);
     m_OutStatsHandles->value(0);
     delete han_list;
     // Automatically go to the first handle
     OnGotoHandle();
     // No need for prev/next if there's only one.
     if(i>1)
     {
       m_MenuPrevHandle->activate();
       m_MenuNextHandle->activate();
     }
     else
     {
       m_MenuPrevHandle->deactivate();
...
void 
UserInterfaceLogic
::OnGotoNextHandleAction()
{
  int curVal = m_OutStatsHandles->value();
  // Check for wrap-around!
  if(curVal>=m_OutStatsHandles->size()-1)
    m_OutStatsHandles->value(0);
  else
    m_OutStatsHandles->value(curVal+1);
  OnGotoHandle();
}

void 
UserInterfaceLogic
::OnGotoPrevHandleAction()
{
  int curVal = m_OutStatsHandles->value();
  // Check for wrap-around!
  if(curVal<1)
    m_OutStatsHandles->value(m_OutStatsHandles->size()-1);
  else
    m_OutStatsHandles->value(curVal-1);
  OnGotoHandle();
}
...
{
  int x,y,z;
  sscanf(m_OutStatsHandles->text(), "%d, %d, %d", &x, &y, &z);
  Vector3ui xCross(x,y,z);
...
::OnFillCavities()
{
  ...
  m_Driver->GetCurrentImageData()->GetSegmentation()->FillCavities(drawing_color, overwrt_color, mode);
  OnSegmentationImageUpdate();
  RedrawWindows();
  OnGatherStatistics();
  this->StoreUndoPoint("Fill cavities");
}
...
::OnDelSatelliteVol()
{
  m_Driver->GetCurrentImageData()->GetSegmentation()->DelSatelliteVol(m_GlobalState->GetDrawingColorLabel());
  OnSegmentationImageUpdate();
  RedrawWindows();
  OnGatherStatistics();
  this->StoreUndoPoint("Delete satellites");
}
...
::OnSmoothLabel()
{
  m_Driver->GetCurrentImageData()->GetSegmentation()->Smooth(m_GlobalState->GetDrawingColorLabel());
  OnSegmentationImageUpdate();
  RedrawWindows();
  OnGatherStatistics();
  this->StoreUndoPoint("Smooth label image");
}
...
::SetToolbarMode(ToolbarModeType mode)
{
  ...
  for(unsigned int i=0;i<3;i++)
    ...
    case TOPOLOGY_MODE:
      m_IRISWindowManager2D[i]->EnterTopologyMode();
      m_TabsToolOptions->value(m_GrpToolOptionTopology);
      break;
...
::OnLoadRecentAction(unsigned int iRecent)
{
  ...
  try
    ...
    typedef itk::Image<GreyLoadType,3> GreyLoadImageType;
    GreyLoadImageType::Pointer imgGrey = io.ReadImage(fnRecent.c_str(), regGrey);
...
::OnMenuSaveGreyROI() 
{
  ...
  // Save the grey image
  // NOTE: this could be bad, as the grey image has been quantized!
  if(m_WizGreyIO->DisplaySaveWizard(
    m_Driver->GetCurrentImageData()->GetGrey()->GetFloatImage(),NULL))
    {
