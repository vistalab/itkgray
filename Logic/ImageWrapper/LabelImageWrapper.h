...
public:
  ...
  /**
   * Compute topology stats on the labels
   */
  void DelSatelliteVol(LabelType currentcolor);
  void FillCavities(LabelType drawing_color, LabelType overwrt_color, CoverageModeType mode);
  void EulerStat(LabelType currentcolor, double *euler, int *nCavities, int *nVolumes, int *nHandles);
  Loc *FindHandles(LabelType currentcolor);

  /** Smooth the label */
  void Smooth(LabelType currentcolor);
...
private:
  unsigned char *vol;
  Loc size;
  bool PrepVol(LabelType currentcolor);
  void DeleteVol();
  unsigned long int nVox;
