...
public:
  ...
  typedef itk::SmartPointer<GreyLoadImageType> GreyLoadImagePointer;
  ...
  /**
   * The grey image can be set directly with its native type 
   * (using our parent's setImage method), or it can be set by 
   * sending a float type using the setFloatImage method. The
   * float image will be scaled and quantized to fit in the 
   * native grey type. 
   */
  void SetFloatImage(GreyLoadImagePointer newFloatImage);
  GreyLoadImagePointer GetFloatImage();
  
  double GetIntensityScale() { return m_intensityScale; };
  double GetIntensityShift() { return m_intensityShift; };
  
  double GetRealImageMin() { return GetImageMin() * m_intensityScale + m_intensityShift; };
  double GetRealImageMax() { return GetImageMax() * m_intensityScale + m_intensityShift; };
  
  inline double GetRealVoxelIntensity(unsigned int x, unsigned int y, unsigned int z);
  
  double GetRealVoxelIntensity(const Vector3ui &index){
    return(m_intensityScale * (double)GetVoxel(index[0],index[1],index[2]) + m_intensityShift);
  }
...
private:
  ...
  /**
   * The intensity of the image data (as stored on disk) is:
   * voxelValue * m_intensityScale + m_intensityShift.
   */
